...
3. Таблица тарифов (Google Sheets + fallback в `config.js`).
   ...
   **Загрузка:** `js/prices.js` → gviz CSV при каждом открытии страницы; кэш 6 ч в `localStorage` — только при ошибке сети. При успешной загрузке таблица перерисовывается сразу.
